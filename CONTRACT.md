...
| Wire compatibility | `contract/goldens/*.jsonl` and `server/tests/protocol-goldens.test.ts`, supplemented by `server/tests/conformance.test.ts` | A current server that rejects an old client message, a current frame that an old reader cannot parse, or a pinned protocol semantic that changes makes a test fail. |

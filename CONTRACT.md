...
already fixed-advance for every script: you give up that guarantee.
Dual-width CJK/emoji pins (`.mtv-w2`) are not behind this switch. `EmbedView`
does not read `sessionPresentation` (composer stays hardcoded COMPOSE; font is
only its explicit `fontPx` prop; no dpad). A standalone `TermView` always pins. Copy does not get a second policy adapter: the
...
2. **Keep a measurable removal window.** Before 1.0, a name deprecated in one

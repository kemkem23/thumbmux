...
| `titleAdornment` | Renders nothing. A supplied `Snippet<[string]>` renders inline on the HUD's session-name row, after the name and before the expand caret, and receives the session name. It is the only slot that can be **withheld**: when the name and the slot cannot both be read on the row, the slot leaves and the name keeps the full width. Use `extraPanel` for anything that must be on screen at every width. | `{#snippet titleAdornment(session)}<b>{elapsed(session)}</b>{/snippet}` |

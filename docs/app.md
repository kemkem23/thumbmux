...
| `labels` | Shallow-merges with `DEFAULT_APP_LABELS`. Every omitted established key keeps its stock English value; function-valued labels such as `hubCount` and `terminalAria` remain functions. The Bash disclosure uses the terse stock labels `BASH`, `SHOW`, `HIDE`, and `DISTILL`; a host that needs localized wording can map that stock action and its additive `choices` through the existing `sessionPresentation.actions` callback. | `labels: { hubTitle: "SESSIONS" }` |

...
| `labels` | Shallow-merges with `DEFAULT_APP_LABELS`. Every omitted established key keeps its stock English value; function-valued labels such as `hubCount` and `terminalAria` remain functions. The package-owned tool disclosure uses the terse stock labels `TOOLS`, `SHOW`, `HIDE`, and Claude-only `DISTILL`; `sessionPresentation.actions` may still place or relabel the trigger, while the mutually-exclusive choices remain outside the frozen `FabAction` adapter type. | `labels: { hubTitle: "SESSIONS" }` |
...
The standalone `TermView` Claude Bash/Codex tool detector and provider-neutral
projection types and helpers are **experimental** (`X` contract tier): the
agent CLIs control their painted terminal layouts, so future minor releases may
need to adjust those low-level shapes.

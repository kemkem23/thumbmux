...
`);
  ...
  emitOutputHook(session, data, cursor, reset, screen) {
...
`);
  ...
  async captureAndBroadcastAsync(session, viewers, opts = {}) {
    ...
    try {
      ...
      if (hash === this.hashes.get(session)) {
        ...
        if (this.hasPendingOutputFrame(session, viewers)) {
          ...
          if ((cursorMoved || screenMoved || archiveReflowGeneration !== undefined) && this.hooks.onOutput) {
            this.emitOutputHook(session, liveContent, cursor2, archiveReflowGeneration !== undefined ? "resize" : undefined, this.lastScreen.has(session) ? this.lastScreen.get(session) ?? null : undefined);
          }
      ...
      if (this.hooks.onOutput) {
        this.emitOutputHook(session, liveContent, cursor, archiveReflowGeneration !== undefined ? "resize" : undefined, rawScreen);
      }
...
function isDuplicateSessionError(error, reservedName) {
  return errorMessage(error).trim().toLowerCase() === `duplicate session: ${reservedName}`.toLowerCase();
}
...
function pathSegment(value) {
  ...
  if (!reserved && cleaned === value)
    return cleaned;
  const digest = createHash4("sha256").update(value).digest("hex").slice(0, 8);
...
`) };
  ...
  readAfter(session, afterLine, limit = 500) {
    ...
    if (state.disabled || state.totalLines === 0)
      return emptyPage();
    const start = Math.max(0, Math.min(Number.isSafeInteger(afterLine) ? afterLine + 1 : 0, state.totalLines));
  ...
  renameSession(oldSession, newSession) {
    ...
    if (!existsSync2(from) || existsSync2(to))
      return;
    try {
      mkdirSync2(dirname3(to), { recursive: true, mode: PRIVATE_DIRECTORY_MODE2 });
      renameSync2(from, to);
    } catch {}
  }
  dropSession(session) {
    this.states.delete(session);
    rmSync2(this.sessionDir(session), { recursive: true, force: true });
  }
  ...
  sessionDir(session) {
    return join4(this.root, pathSegment(this.groupOf(session)), pathSegment(session));
  }
  stateFor(session) {
    const cached = this.states.get(session);
    if (cached)
...
`) + 1;
  ...
  append(state, lines) {
    mkdirSync2(state.dir, { recursive: true, mode: PRIVATE_DIRECTORY_MODE2 });
    let index = 0;
...
`)}
`;
      const path = join4(state.dir, chunk.file);
      const fresh = !existsSync2(path);
      appendFileSync(path, payload);
      if (fresh)
        chmodSync2(path, PRIVATE_FILE_MODE2);
  ...
  appendIndex(state, chunk) {
    try {
      const path = join4(state.dir, "index.jsonl");
      const fresh = !existsSync2(path);
...
`);
  ...
  writeMeta(session, state) {
    const path = join4(state.dir, "meta.json");
    ...
    writeFileSync2(tmp, body, { mode: PRIVATE_FILE_MODE2 });
  ...
  readRange(state, start, end) {
    ...
    for (const chunk of state.chunks) {
      ...
      if (chunkEnd <= start || chunk.start >= end)
        continue;
      const text = readFileSync2(join4(state.dir, chunk.file), "utf8");

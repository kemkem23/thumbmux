...
__export(exports_transfer, {
  ...
  isHostChunksOrphanJson: () => isHostChunksOrphanJson,
  importHistorySnapshot: () => importHistorySnapshot,
  exportHistoryBundle: () => exportHistoryBundle,
  HOST_CHUNKS_SIDECARS: () => HOST_CHUNKS_SIDECARS
});
...
function isHostChunksOrphanJson(name, listed) {
...
function parseSource(input, files) {
  ...
  try {
    if (input.format === "file-jsonl") {
      ...
      if ([...files.keys()].some((n) => isHostChunksOrphanJson(n, listed)))
...
var HOST_CHUNKS_SIDECARS;
var init_transfer = __esm(() => {
  init_codec();
  HOST_CHUNKS_SIDECARS = new Set(["lifecycle-binding.json"]);
});
...
function readSealedHistoryOracle(directory, format) {
  ...
  if (format === "file-jsonl") {
    ...
    if ([...files.keys()].some((name) => isHostChunksOrphanJson(name, listed)))
...
__export(exports_rollout, {
  ...
  HistoryRolloutRouter: () => HistoryRolloutRouter,
...
class HistoryRolloutAllowlist {
  ...
  guardSession(sessionId) {
    if (this.routeSession(sessionId) !== "sqlite-authoritative")
      return "legacy";
    const group = this.store.session(sessionId).group_label;
    ...
    try {
      readMirror(this.mirrorDirectory, sessionId);
    } catch (error) {
  ...
  loseMirror(group, sessionId, reason, observed) {
...
class HistoryRolloutRouter {
  store;
  allowlist;
  writers;
  constructor(store, allowlist, writers) {
    this.store = store;
  ...
  async commitBatch(batch) {
    ...
    if (route === "sqlite-authoritative")
      return this.writers.sqlite.commitBatch(batch);
    return this.writers.legacy.commitBatch(batch);
  }
}
async function expandGroup(store, allowlist, group, options) {
  const startedAt = Date.now();
  const evidence = await assessGroupReadiness(store, group, options.mirrorDirectory);
  const state = allowlist.enableGroup(evidence);
  const drills = [];
  try {
    const scratch = privateDirectory2(options.scratchDirectory);
    for (const sessionId of state.evidence.expectedSessions) {
      const bundle = join3(scratch, `bundle-${sha(sessionId)}-${randomUUID6()}`);
...
async function createSqliteHistoryStore(options) {
  ...
  return {
    ...
    createRolloutRouter: (o) => new rollout.HistoryRolloutRouter(store, o.allowlist, { sqlite: o.sqlite, legacy: o.legacy }),
    assessGroupReadiness: (group, mirrorDirectory) => rollout.assessGroupReadiness(store, group, mirrorDirectory),
    expandGroup: (allowlist, group, o) => rollout.expandGroup(store, allowlist, group, o),

...
import { parseReplayJournal as parseReplayJournal2 } from "@thumbmux/core";
...
import { chmodSync as chmodSync2, existsSync, lstatSync as lstatSync2, mkdirSync as mkdirSync2, openSync as openSync2, closeSync as closeSync2, fsyncSync as fsyncSync2 } from "node:fs";
import { dirname as dirname2, resolve } from "node:path";
import { randomUUID as randomUUID3 } from "node:crypto";
import { parseReplayJournal as parseReplayJournal3 } from "@thumbmux/core";
...
class HistoryStore {
  ...
  report(sid, detector, expected, observed, missing = null) {
    const fault = { issue_id: randomUUID3(), sessionId: sid, detector, expected, observed, timestamp: Date.now(), missing_count: missing };
  ...
  async register(input) {
    return this.write("", () => {
      ...
      const sid = randomUUID3(), floor = safe(input.firstLine ?? 0);
  ...
  ticket(sid, requestId = randomUUID3()) {
    this.checkOwner();
...
function prepareFile(file) {
  const path = resolve(file), dir = dirname2(path);
  if (!existsSync(dir))
    mkdirSync2(dir, { recursive: true, mode: 448 });
  const d = lstatSync2(dir);
...
class HistoryCoordinator {
  ...
  async collect(sid) {
    try {
      ...
      const ticket = this.store.ticket(sid, randomUUID4());
...
import { basename as basename3, join as join2, resolve as resolve2 } from "node:path";
...
class BridgeSpool {
  ...
  path(requestId) {
    return join2(this.directory, `${sha(requestId)}.json`);
  }
  readPath(path) {
    ...
    const entry = JSON.parse(readFileSync2(path, "utf8"));
    if (entry.version !== 1 || !entry.requestId || basename3(path) !== `${sha(entry.requestId)}.json`)
  ...
  list() {
    return readdirSync2(this.directory).sort().map((name) => {
      if (!/^[a-f0-9]{64}\.json$/.test(name))
        throw new Error("unexpected-bridge-spool-entry");
      return this.readPath(join2(this.directory, name));
    });
  }
  accept(batch) {
  ...
  write(entry, replace) {
    const target = this.path(entry.requestId), temporary = join2(this.directory, `.${sha(entry.requestId)}-${randomUUID5()}.pending`);
...
async function createSqliteHistoryStore(options) {
  const [{ Database }, { HistoryStore: HistoryStore2, prepareFile: prepareFile2 }, { HistoryCoordinator: HistoryCoordinator2 }, { OptInHistoryBridge: OptInHistoryBridge2 }, transfer, rehearsal] = await Promise.all([
    import("bun:sqlite"),

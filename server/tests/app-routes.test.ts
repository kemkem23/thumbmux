  ...
  test("v0.7.1 send-only WsLike host compiles against the current declaration", async () => {
    ...
    try {
      ...
      await Bun.write(consumerPath, [
        ...
        "}",
        "",
        "// V4 attack: v0.7.1 had no close on WsLike, so an adapter carrying the",
        "// standard WebSocket close(code, reason) was assignable. Declaring a",
        "// zero-arg close?() on the frozen type stops it compiling — a function",
        "// requiring two arguments cannot satisfy one callable with none.",
        "class LegacyAdapterSocket {",
        "  send(data: string): number {",
        "    return data.length;",
        "  }",
        ...
        "const legacy: WsLike = new LegacyAdapterSocket();",
        "socket.send(\"compile probe\");",
        "legacy.send(\"compile probe\");",
        "",
      ].join("\n"));

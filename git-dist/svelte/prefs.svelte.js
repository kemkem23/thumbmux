...
export function createServerPrefs(opts) {
    ...
    const subs = new Set();
    const emit = (p) => { writeCache(cacheKey, p); for (const cb of subs)
        cb(p); };
    ...
    return {
        async load() {
            ...
            doFetch(url).then(async (r) => {
                ...
                if (fresh && typeof fresh === 'object' && JSON.stringify(fresh) !== JSON.stringify(readCache(cacheKey)))
                    emit(fresh);
            }).catch(() => { });
            return cached;
        },
        async save(patch) {
            ...
            try {
                ...
                if (r.ok && generation === gen) {
                    ...
                    if (saved && typeof saved === 'object' && generation === gen)
                        emit(saved);
                }
            }

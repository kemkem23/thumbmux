...
function parsePipehistIdentityV1(value, label = "identity") {
  if (!isObject(value))
    throw new Error(`${label} must be an object`);
...
function parsePipehistProxyConfigV1(value) {
  if (!isObject(value))
    throw new Error("pipehist must be an object");
  exactKeys(value, ["schema", "identity", "replay"], [], "pipehist");
  if (value.schema !== "pipehist.c.v1/proxy-config")
    throw new Error("pipehist.schema is invalid");
  let replay = null;
  if (value.replay !== null) {
    if (!isObject(value.replay))
      throw new Error("pipehist.replay must be an object or null");
...
function parseTerminalPtyWalProxyConfig(value) {
  ...
  if (pipehist && (pipehist.identity.session !== parsedIdentity.session || pipehist.identity.instanceId !== parsedIdentity.instanceId)) {
...
function parseHeader(header, offset, previousSequence, previousAt, maxPayloadBytes, expectedVersion) {
  ...
  try {
    ...
    while (offset < stat.size && records.length < maxRecords) {
      ...
      if (parsed.kind === "gap") {
        ...
        if (BigInt(gap.lastDurableSeq) !== parsed.sequence - 1n)
          throw new Error("invalid gap boundary");
      }
...
function parseOutputWalGapPayload(payload) {
  const value = JSON.parse(Buffer.from(payload).toString("utf8"));
  if (typeof value !== "object" || value === null)
    throw new Error("invalid WAL gap");
  const gap = value;
  if (typeof gap.gapId !== "string" || !gap.gapId || typeof gap.sourceEpoch !== "string" || !gap.sourceEpoch || typeof gap.paneId !== "string" || !/^%[0-9]+$/.test(gap.paneId) || typeof gap.lastDurableSeq !== "string" || !/^(0|[1-9][0-9]*)$/.test(gap.lastDurableSeq) || gap.reason !== "tmux-pause" && gap.reason !== "recorder-failure" && gap.reason !== "unclean-source" || gap.missingBytes !== null || gap.coverage !== "unknown" || typeof gap.detectedAt !== "number" || !Number.isSafeInteger(gap.detectedAt) || gap.detectedAt < 0) {
    throw new Error("invalid WAL gap: unknown loss must remain null/unknown");
  }
  return gap;
}
function parseOutputWalRecoveryPayload(payload) {
  const value = JSON.parse(Buffer.from(payload).toString("utf8"));
  if (typeof value !== "object" || value === null)
    throw new Error("invalid WAL recovery snapshot");
  const recovery = value;
  const identity = recovery.identity;
  const geometry = recovery.geometry;
  if (typeof recovery.gapId !== "string" || !recovery.gapId || typeof recovery.sourceEpoch !== "string" || !recovery.sourceEpoch || typeof recovery.paneId !== "string" || !/^%[0-9]+$/.test(recovery.paneId) || recovery.provenance !== "recovered-from-ring" || recovery.status !== "success" && recovery.status !== "failed" && recovery.status !== "ambiguous" || typeof recovery.recoveredBytesBase64 !== "string" || recovery.recoveredRows !== null && (!Number.isSafeInteger(recovery.recoveredRows) || recovery.recoveredRows < 0 || recovery.recoveredRows > 1e4) || recovery.truncated !== null && typeof recovery.truncated !== "boolean" || recovery.boundary !== null && recovery.boundary !== "matched" && recovery.boundary !== "ambiguous" || recovery.error !== undefined && typeof recovery.error !== "string" || typeof recovery.capturedSeqBefore !== "string" || !/^(0|[1-9][0-9]*)$/.test(recovery.capturedSeqBefore) || typeof recovery.capturedSeqAfter !== "string" || !/^(0|[1-9][0-9]*)$/.test(recovery.capturedSeqAfter) || BigInt(recovery.capturedSeqAfter) < BigInt(recovery.capturedSeqBefore) || !identity || typeof identity.session !== "string" || typeof identity.sessionId !== "string" || typeof identity.windowId !== "string" || identity.paneId !== recovery.paneId || typeof identity.paneTarget !== "string" || !Number.isSafeInteger(identity.tmuxServerPid) || !Number.isSafeInteger(identity.sessionCreated) || !geometry || !Number.isSafeInteger(geometry.cols) || geometry.cols <= 0 || !Number.isSafeInteger(geometry.rows) || geometry.rows <= 0) {
    throw new Error("invalid WAL recovery snapshot provenance or boundary");
  }
  if (recovery.status === "success" && (recovery.boundary !== "matched" || recovery.truncated !== false || recovery.recoveredRows === null || recovery.error !== undefined) || recovery.status === "ambiguous" && (recovery.recoveredRows === null || recovery.truncated === null || recovery.boundary === null || recovery.error !== undefined) || recovery.status === "failed" && (recovery.recoveredRows !== null || recovery.truncated !== null || recovery.boundary !== null || recovery.recoveredBytesBase64 !== "" || typeof recovery.error !== "string")) {
  ...
  if (recovered.byteLength > 8 * 1024 * 1024 || recovered.toString("base64") !== recovery.recoveredBytesBase64) {
    ...
    try {
      switch (record.kind) {
        ...
        case "gap": {
          ...
          if (this.pendingGaps.has(gap.gapId))
            throw new Error(`duplicate pending gap ${gap.gapId}`);
...
`));
          if (recovery.status !== "failed") {
            const rows = stripVTControlCharacters(Buffer.from(recovery.recoveredBytesBase64, "base64").toString("utf8")).replace(/[\x00-\x09\x0b-\x1f\x7f-\x9f]/g, "");
            if (rows)
              onHistory(Buffer.from(rows.endsWith(`
`) ? rows : `${rows}
`));
          }
...
`));
        ...
        case "checkpoint": {
          const value = parseOutputWalJson(record);
          if (isObject2(value) && (value.event === "source-tracking" || value.event === "source-detached")) {
            this.requireActive(record);
            const keys = value.event === "source-tracking" ? "event,version" : "event,lastDurableSeq,version";
            if (value.version !== 1 || Object.keys(value).sort().join(",") !== keys || value.event === "source-detached" && value.lastDurableSeq !== (record.sequence - 1n).toString()) {
...
function terminalReplayResultFromWire(value) {
  ...
  return {
    ...
    ...value.lastRecordAt === undefined ? {} : { lastRecordAt: boundedInteger2(value.lastRecordAt, `${label}.lastRecordAt`, 0, Number.MAX_SAFE_INTEGER) },
...
function parseRequest(value) {
  ...
  const id2 = boundedString(value.id, `${label}.id`, 64);
  if (!/^[A-Za-z0-9_-]+$/.test(id2))
  ...
  return { protocol: 1, id: id2, command: value.command };
}
...
class ProcessTerminalReplayWorkerClient {
  ...
  send(command) {
    ...
    const id2 = `r${this.nextId.toString(36)}`;
    this.nextId += 1n;
...
`);
  ...
  screenEq(a, b) {
    ...
    if (x === null || y === null)
      return x === y;
    return x.alt === y.alt && x.mouseSgr === y.mouseSgr && x.mouseAny === y.mouseAny && x.previewState === y.previewState && x.previewHasFrame === y.previewHasFrame && x.previewLastSuccessfulAt === y.previewLastSuccessfulAt;
  }
  boundaryEq(a, b) {
...
`);
  ...
  async captureAndBroadcastAsync(session, viewers, opts = {}) {
    ...
    try {
      ...
      const startLine = opts.fullHistory ? -Math.max(this.driver.getHistoryLimit(session), this.liveLineLimit) : this.captureStartLines.get(session) ?? this.DEFAULT_CAPTURE_START_LINE;
...
var DEFAULT_DRIVER_HISTORY_LIMIT = 2000;
function run(args) {
...
`)) {
    ...
    getHistoryLimit(session) {
      if (session !== undefined) {
        try {
          const m = run(["display-message", "-p", "-t", target.pane(session), "#{history_limit}"]).match(/(\d+)/);
          const limit = m ? Number(m[1]) : 0;
          return limit > 0 ? limit : DEFAULT_DRIVER_HISTORY_LIMIT;
        } catch {
          return DEFAULT_DRIVER_HISTORY_LIMIT;
        }
      }
      try {

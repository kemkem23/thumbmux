...
export interface HistoryEvidence {
    ...
    depth: 'shallow' | 'deep' | 'import';
    source: SourceObservation;
    rawSha256: string;
    requestDigest: string;
    importSpan?: {
        source_id: string;
        physicalRecordStart: number;
        count: number;
    };
    legacy?: {
        liveStart: number;
...
export interface SqliteHistoryOptions {
    file: string;
    onFault?: (fault: HistoryFault) => void;
}
...
export interface CaptureBatch {
    ...
    evidence: Omit<HistoryEvidence, 'requestDigest'>;
    recordFrames?: boolean;
    recordingSessionBytes?: number;
    recordingRootBytes?: number;
...
export interface ShadowFrameRecord {
    ordinal: number;
    bytes: string;
}
export interface ShadowUnresolvedRecord {
    ordinal: number;
    sha256: string;
}
export interface ShadowBatchSnapshot {
    ...
    revision: number;
    rows: HistoryRow[];
    frames: ShadowFrameRecord[];
    unresolved: ShadowUnresolvedRecord[];
}
...
export interface ShadowComparisonReport {
    ...
    receipts: {
        ...
        sqliteRequestId: string;
        match: boolean;
    };
    lines: {
        matchedCoordinates: number;
...
export interface LegacyProjectionAcknowledgement {
    requestId: string;
    digest: string;
    shadow?: ShadowBatchSnapshot;
}
...
export interface HistoryBridgeOptions extends HistoryCoordinatorOptions {
    ...
    legacyProjection: LegacyProjectionWriter;
}
export interface HistoryBridgeLedgerEntry {
    ...
    sqliteRevision: number | null;
    shadowCompared?: boolean;
    shadowDelivered?: boolean;
}
export interface DualWriteReceipt {
    sqlite: CaptureReceipt;
    requestId: string;
    digest: string;

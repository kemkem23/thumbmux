import type { AnsiPalette, ClaudeBashSummaryRequest, LaunchPreset, LaunchSpec, PreferencesAdapter, SessionListItem, SubmitAgent, TerminalSurfaceWithPalette, UploadedFile } from '../core/index.js';
import type { FabAction, GridFilterOption, GridOrder, GridSession, LaunchContext, TmuxMux } from '../svelte/index.js';
...
export interface AppAdapters {
    ...
    bashSummaries?: (session: string, requests: readonly ClaudeBashSummaryRequest[]) => Promise<Readonly<Record<string, string>>>;
    upload?: {
        endpoint(session: string): string | null;
        dir?: string;
        /** Add multipart fields and return opaque context tied to this exact request. */
        prepareForm?: (session: string, files: readonly File[], form: FormData) => unknown | Promise<unknown>;
        /** Observe parsed HTTP state with the exact per-request context. */
        onResponse?: (session: string, files: readonly File[], response: Response, data: unknown, context: unknown) => void | Promise<void>;

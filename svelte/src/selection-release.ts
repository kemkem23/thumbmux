 ...
 */
export const STOCK_SCROLL_CONTROL_SELECTORS: readonly string[] = [
...
type ResolvedOptions = {
  scrollControls: string;
  handleMarginPx: number;
};

function resolveOptions(options: SelectionReleaseOptions): ResolvedOptions {
...
function endpointsOf(selection: Selection): Element[] {
  const anchor = elementOf(selection.anchorNode);
  const focus = elementOf(selection.focusNode);
  return [anchor, focus].filter((el): el is Element => el !== null);
}
 ...
 */
export function releasableTerminalSelection(): { selection: Selection; range: Range } | null {
  ...
  if (
    endpoints.some(
      (el) => el.closest(EDITABLE_SELECTOR) || el.closest(TERMINAL_SEARCH_SELECTOR),
    )
  ) return null;
  if (!endpoints.some((el) => el.closest(TERMINAL_VIEWPORT_SELECTOR))) return null;

  return { selection, range: selection.getRangeAt(0) };
}

function release(selection: Selection): void {
  try {
 ...
 */
export function pointIsOnSelection(
  x: number,
  y: number,
  range: Range,
  marginPx: number = SELECTION_HANDLE_MARGIN_PX,
): boolean {
  for (const rect of range.getClientRects()) {
    if (
      x >= rect.left - marginPx && x <= rect.right + marginPx
...
export function isScrollIntent(
  ...
  scrollControlSelector: string = STOCK_SCROLL_CONTROL_SELECTORS.join(', '),
): boolean {
  const element = elementOf(target as Node | null);
  if (!element) return false;
  if (element.closest(TERMINAL_VIEWPORT_SELECTOR)) return true;
  return scrollControlSelector.length > 0 && Boolean(element.closest(scrollControlSelector));
}

/**
 * Installs are reference-counted per resolved configuration, so a host that
 * calls this from two mounted layouts adds one pair of listeners, not two.
 * The listeners come off when the last disposer for that configuration runs;
 * a disposer called twice does not decrement twice.
 */
type Installation = { count: number; remove: () => void };
 ...
 */
export function installTerminalSelectionRelease(
  options: SelectionReleaseOptions = {},
): () => void {
...
function disposerFor(key: string, installation: Installation): () => void {
  let disposed = false;
  return () => {
    if (disposed) return;
    disposed = true;

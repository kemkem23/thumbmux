...
var BEL = "\x07";
var ST = "\\";
function isBlankToolSeparator(raw) {
  let index = 0;
  while (index < raw.length) {
    ...
    if (next !== "[")
      return false;
    let end = index + 2;
    while (end < raw.length && /[0-9:;]/u.test(raw[end] ?? ""))
      end += 1;
    if (raw[end] !== "m")
      return false;
    index = end + 1;
...
var CLAUDE_ELAPSED_ONLY_DETAIL = "\\d+[hms](?: \\d+[ms]){0,2}";
...
var CLAUDE_ACTIVITY_MARKER_FG = 174;
var CLAUDE_ACTIVITY_VERB_FG = 174;
var CLAUDE_ACTIVITY_VERB_SPARKLE_FG = 216;
var CLAUDE_ACTIVITY_METADATA_FG = 246;
function isStyledClaudeActivityStatusLine(raw, initialForeground = null, initialInverse = false, initialConcealed = false) {
  ...
  if (verbStart < 0 || metadataStart < 0 || metadataEnd <= metadataStart)
    return false;
  const paintedAs = (start, end, allowed) => {
    for (let unit = start;unit < end; unit += 1) {
      if (/\s/u.test(normalized[unit] ?? ""))
        continue;
      const foreground = paint.foregrounds[unit] ?? null;
      if (!allowed.includes(foreground))
        return false;
    }
    return true;
  };
  const currentPaint = paintedAs(0, marker.length, [CLAUDE_ACTIVITY_MARKER_FG]) && paintedAs(verbStart, verbStart + verb.length, [
...
function isMuxPaneScreen(value) {
  ...
  return typeof screen.alt === "boolean" && typeof screen.mouseSgr === "boolean" && typeof screen.mouseAny === "boolean" && (previewLastSuccessfulAt === undefined || typeof previewLastSuccessfulAt === "number" && Number.isFinite(previewLastSuccessfulAt) && previewLastSuccessfulAt >= 0);
}
...
var USER_REPORT_END_MARKER = "<!-- thumbmux:user-report:end -->";
function isLegacyUserReportBoundary(line) {
  const trimmed = line.trim();
  return /^##\s+(?:Attached screenshots|Source)\s*$/i.test(trimmed) || /^##\s+Workflow\s+\(execute end-to-end, do not pause\)\s*$/i.test(trimmed);
}
function extractMarkdownSection(lines, title, preserveNewlines) {
  ...
  for (const line of lines.slice(start + 1)) {
    if (line.trim() === USER_REPORT_END_MARKER || isLegacyUserReportBoundary(line))
      break;
    section.push(line);
  }
  const text = preserveNewlines ? section.join(`
`).trim() : section.join(" ").replace(/\s+/g, " ").trim();
  return text || null;
}
function normalizePromptBlock(lines, wrapJoined) {
...
function collectPrompts(lines, start, matchers, wrapJoined) {
  const prompts = [];
  let i = start;
...
function isLiveRunningResult(line) {
  if (!isResultDelimiter(line))
    return false;
...
function parseCandidate(rawLines, startLine, header, limits) {
  ...
  if (header.status === "completed") {
    if (delimiterLine < 0 || endLine < 0) {
      liveRunningUnclosed = delimiterLine >= 0 && endLine < 0 && ambiguousActivityStatusLine < 0 && isLiveRunningResult(visibleLine(rawLines[delimiterLine] ?? ""));
      if (!liveRunningUnclosed)
        return { block: null };
  ...
  const status = liveRunningUnclosed ? "active" : header.status === "active" && delimiterLine >= 0 && completionBoundary ? "completed" : header.status;
...
function projectionRowsCoverRawSource(projection) {
  let cursor = 0;
  const memberKeys = new Set;
  for (const row of projection.rows) {
    if (!validRange(row.rawRange, projection.rawLines.length) || row.rawStart !== row.rawRange.startLine || row.rawEndExclusive !== row.rawRange.endLine || row.rawRange.startLine !== cursor)
      return false;
    if (row.kind === "tool-placeholder") {
      if (!row.block || !row.placeholderKey || memberKeys.has(row.placeholderKey))
        return false;
      memberKeys.add(row.placeholderKey);
    } else if (row.block !== null || row.placeholderKey !== null)
      return false;
    cursor = row.rawRange.endLine;
  }
  return cursor === projection.rawLines.length;
}
function rangeTouchesAny(startLine, endLine, ranges) {
  return ranges.some((candidate) => candidate.startLine < endLine && startLine < candidate.endLine);
}
function gapCrossesBarrier(startLine, endLine, barrierLines) {
  for (const barrier of barrierLines) {
    if (barrier >= startLine && barrier <= endLine)
      return true;
  }
  return false;
}
function joinableToolPlaceholderGap(projection, rows, previousIndex, nextIndex, barriers, hardRanges) {
  ...
  if (gapStart > gapEnd || gapCrossesBarrier(gapStart, gapEnd, barriers))
    return false;
  if (rangeTouchesAny(gapStart, gapEnd, hardRanges))
    return false;
  for (let index = previousIndex + 1;index < nextIndex; index += 1) {
    ...
    for (let rawRow = row.rawRange.startLine;rawRow < row.rawRange.endLine; rawRow += 1) {
      if (!isBlankToolSeparator(projection.rawLines[rawRow] ?? ""))
        return false;
    }
  }
  return true;
}
function validPreviousToolGroups(groups) {
  const keyOwners = new Map;
  for (const group of groups) {
    if (!group.placeholderKey.trim())
      continue;
    keyOwners.set(group.placeholderKey, (keyOwners.get(group.placeholderKey) ?? 0) + 1);
  }
  return groups.filter((group) => group.placeholderKey.trim() !== "" && keyOwners.get(group.placeholderKey) === 1 && group.line !== "" && group.memberPlaceholderKeys.length > 0 && new Set(group.memberPlaceholderKeys).size === group.memberPlaceholderKeys.length && group.memberPlaceholderKeys.every((key) => key.trim() !== "") && Number.isSafeInteger(group.rawRange.startLine) && Number.isSafeInteger(group.rawRange.endLine) && group.rawRange.startLine < group.rawRange.endLine && Number.isSafeInteger(group.blockCount) && group.blockCount > 0);
}
function assignToolPlaceholderGroupKeys(candidates, previousGroups) {
  const previous = validPreviousToolGroups(previousGroups);
  const ownerByMemberKey = new Map;
  const ambiguousMemberKeys = new Set;
  previous.forEach((group, previousIndex) => {
    for (const key of group.memberPlaceholderKeys) {
      if (ownerByMemberKey.has(key))
        ambiguousMemberKeys.add(key);
      else
        ownerByMemberKey.set(key, previousIndex);
    }
  });
  for (const key of ambiguousMemberKeys)
    ownerByMemberKey.delete(key);
  const assigned = Array(candidates.length).fill(null);
  let previousFloor = -1;
  candidates.forEach((candidate, candidateIndex) => {
    const overlappingPrevious = new Set;
    for (const memberKey of candidate.memberPlaceholderKeys) {
      const previousIndex = ownerByMemberKey.get(memberKey);
      if (previousIndex === undefined || previousIndex <= previousFloor)
        continue;
    ...
    for (const previousIndex of overlappingPrevious) {
      if (chosenIndex === undefined || previousIndex < chosenIndex)
        chosenIndex = previousIndex;
    }
  ...
  const usedKeys = new Set(assigned.filter((key) => key !== null));
  const reservedPreviousKeys = new Set(previous.map(({ placeholderKey }) => placeholderKey));
  return candidates.map((candidate, index) => {
    const carried = assigned[index];
    if (carried !== null)
      return carried;
    const base = candidate.memberPlaceholderKeys[0] ?? `tool-placeholder:${candidate.provider}:group-${index}`;
    let key = base;
    let suffix = 1;
...
function groupedIdentityProjection2(projection) {
  return {
    ...projection,
...
function coalesceAdjacentToolProjection(projection, options = {}) {
  if (projection.rows.length === 0 || !projectionRowsCoverRawSource(projection)) {
    return groupedIdentityProjection2(projection);
  }
  const barriers = new Set((options.barrierLines ?? []).filter((line) => Number.isSafeInteger(line) && line >= 0 && line <= projection.rawLines.length));
  const hardRanges = [
    ...projection.projectedBlocks.flatMap((block) => block.protectedRanges),
    ...projection.rejectedBlocks.map(({ block }) => block.sourceRange).filter((candidate) => validRange(candidate, projection.rawLines.length))
  ];
  const placeholderIndexes = projection.rows.flatMap((row, index) => row.kind === "tool-placeholder" ? [index] : []);
  if (placeholderIndexes.length === 0)
    return groupedIdentityProjection2(projection);
  const candidates = [];
  for (const rowIndex of placeholderIndexes) {
    const row = projection.rows[rowIndex];
    if (!row?.block || !row.placeholderKey)
      return groupedIdentityProjection2(projection);
    const current = candidates.at(-1);
    if (current && current.provider === row.block.provider && current.line !== "" && current.line === row.line && joinableToolPlaceholderGap(projection, projection.rows, current.endRowIndex, rowIndex, barriers, hardRanges)) {
  ...
  const groupKeys = assignToolPlaceholderGroupKeys(candidates, options.previousGroups ?? []);
  const groupByStartRow = new Map;
  candidates.forEach((candidate, index) => {
    ...
    const group = Object.freeze({
      ...
      line: candidate.line,
      rawRange: range2(first.rawRange.startLine, last.rawRange.endLine),
  ...
  const rows = [];
  const rawToVisualRow = Array(projection.rawLines.length);
  let hiddenLineCount = 0;
...
function updateShellQuotes(raw, quotes) {
...
function commandDetailRows(header) {
  ...
  return {
    accepts: (raw, _bodyOffset) => {
      ...
      if (currentlyInQuotes && evidence.hasSemanticCell && hasOnlyCompleteTerminalControls(raw)) {
        return true;
      }
      return false;
    },
    isContinuationDetail: (raw) => {
...
function leadingBoundaryIsKnown(rawLines, line, scanStart, leadingEdgeSealed, previousCompletedEndLine) {
  if (line === previousCompletedEndLine)
    return true;
  if (line === 0)
...
function isProtectedSemanticRow(raw, allowDimBoundaryPrefix = false) {
  ...
  if (/^(?:■|⚠|Warning\b|Error\b|FAILED\b|Approval\b|Approve\b|Allow\b|Deny\b|Do you want\b)/iu.test(line)) {
    if (!(allowDimBoundaryPrefix && DIM_ROW.test(raw)))
      return true;
  }
...
function sealedBody(rawLines, startLine, maxBlockLines, maxBlockChars, acceptsRow, allowDimBoundaryContinuation = false, isContinuationDetail) {
  ...
  for (let line = startLine + 1;line < maximumEnd; line += 1) {
    ...
    if (isSealRow(raw)) {
      if (isContinuationDetail) {
        ...
        if (nextLine < maximumEnd) {
          ...
          const nextIsProtected = isProtectedSemanticRow(nextRaw, allowDimBoundaryContinuation && bodyLines.length > 0);
    ...
    if (candidateChars > maxBlockChars)
      return null;
    if (isProtectedSemanticRow(raw, allowDimBoundaryContinuation && bodyLines.length > 0) || !acceptsRow(raw, bodyLines.length))
      return null;
    bodyLines.push(raw);
  }
  return null;
...
function parseAt(rawLines, startLine, maxBlockLines, maxBlockChars) {
  ...
  if (waited) {
    ...
    const body = sealedBody(rawLines, startLine, maxBlockLines, maxBlockChars, detail.accepts, true, detail.isContinuationDetail);
    if (!body)
      return null;
...
function detectCodexToolBlocks(rawLines, options = {}) {
  ...
  const blocks = [];
  let previousCompletedEndLine;
  for (let line = scanStart;line < rawLines.length; line += 1) {
    if (!leadingBoundaryIsKnown(rawLines, line, scanStart, options.leadingEdgeSealed === true, previousCompletedEndLine))
      continue;
...
var THEME_MODES = ["dark", "light"];
function isThemeMode(value) {
  return value === "dark" || value === "light";
}
function isThemeBackground(value) {
  return typeof value === "string" && /^#[0-9a-f]{6}$/i.test(value);
}
function emptyThemeBackgrounds() {
  return { dark: {}, light: {} };
}
function parseThemeBackgrounds(raw) {
...
function parseThemeBackgroundsJson(raw) {
  if (!raw)
...
function readThemeBackground(backgrounds, mode, key) {
  const value = backgrounds[mode]?.[key];
  return isThemeBackground(value) ? value : null;
}
function writeThemeBackground(backgrounds, mode, key, hex) {
  const next = { ...backgrounds[mode] };
  if (hex !== null && isThemeBackground(hex))
    next[key] = hex;
  else
    delete next[key];
...
function isExtraEnterAgent(agent) {
  return agent === `${"co"}${"dex"}` || agent === `${"clau"}${"de"}` || agent === `${"gr"}${"ok"}`;
}
...
// src/clear-context.ts
var CLEAR_CONTEXT_CONTROL_BYTES = {
  "C-u": "\x15",
  Escape: "\x1B"
};
var DEFAULT_CLEAR_COMMAND = "/clear";
var DEFAULT_SETTLE_MS = 120;
var DEFAULT_SECOND_ENTER_MS = 1000;
var DOUBLE_ENTER_AGENT = `${"co"}${"dex"}`;
function clearContextStrategy(agent) {
  return agent === DOUBLE_ENTER_AGENT ? "codex-double-enter" : "single-submit";
}
function clearContextPlan(opts = {}) {
...
// src/text-query.ts
function matchesTextQuery(text, query, options) {

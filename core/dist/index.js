...
function collectTerminalUrlSegments(rawLines, startLine, endLine, cols) {
  ...
  for (let wi = startLine;wi < endLine; wi++) {
    ...
    while ((match = urlStartRe.exec(stripped)) !== null) {
      ...
      while (curEndPos >= cols - 2 && curEndPos > 10 && curIdx + 1 < continuationLimit && segments.length - 1 < MAX_CONTINUATION_ROWS) {
...
function keyboardEventToSequence(e, opts = {}) {
  ...
  if (arrowFinal) {
    return shifted || alt || ctrl ? modifiedCsi(arrowFinal, shifted, alt, ctrl) : `${ESC3}[${arrowFinal}`;
  }
  const homeEndFinal = homeEndFinals[key];
  if (homeEndFinal) {
    return shifted || alt || ctrl ? modifiedCsi(homeEndFinal, shifted, alt, ctrl) : `${ESC3}[${homeEndFinal}`;
  }
  if (key === "Enter")
    return alt ? `${ESC3}\r` : "\r";
  if (key === "Tab") {

...
function claudeComposerRuleCells(raw) {
  const style = /^\x1b\[38(?:;5;244|:5:244)m/.exec(raw);
  if (!style)
    return null;
  if (raw.slice(style[0].length).includes("\x1B"))
    return null;
...
function confirmedClaudeComposerRuleAt(rawLines, index) {
  ...
  for (const otherIndex of [index + 2, index - 2]) {
    ...
    if (otherCells === cells && /^❯(?:\s|$)/.test(middle))
      return cells;
  }
  return null;
}
function resumeAfterAmbiguousCorridor(rawLines, ambiguousLine, scanEnd) {
  for (let index = Math.min(rawLines.length, scanEnd) - 1;index > ambiguousLine; index -= 1) {
    if (confirmedClaudeComposerRuleAt(rawLines, index) !== null)
      return index;
  }
  return scanEnd;
}
function inferredPaneColumns(rawLines) {
...
function isCalibratedClaudeTopLevel(raw) {
  return /^(?:\x1b\[0m)?\x1b\[38(?:;5;|:5:)(?:114|231|246|211|220)m(?:\x1b\[49m)?(?:●|✻)\x1b\[39m(?:\s|$)/.test(raw);
}
...
function parseCandidate(rawLines, startLine, header, limits) {
    ...
    if (boundary) {
      ...
      if (delimiterLine >= 0) {
        if (boundary === "approval")
          return { block: null };
        const calibratedTopLevel = boundary === "top-level" && isCalibratedClaudeTopLevel(raw);
        ...
        const rejectedAmbiguousBoundary = nextHeader ? { block: null, resumeLine: resumeAfterAmbiguousCorridor(rawLines, i, scanEnd) } : { block: null };
  ...
  const status = header.status === "active" && delimiterLine >= 0 && boundaryLine >= 0 ? "completed" : header.status;

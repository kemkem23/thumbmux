...
function boundaryKind(line) {
  if (/^\s*(?:I want to run:|Do you want to (?:allow|proceed)|Would you like to (?:allow|proceed)|This command requires approval)/i.test(line) || /^❯\s*\d+[.)]\s*(?:Yes|No|Allow|Deny)\b/i.test(line))
    return "approval";
  if (/^(?:●|⏺|❯|✻)(?:\s|$)/.test(line))
    return "top-level";
  if (/^[─━-]{8,}\s*$/.test(line))
...
function placeholderLine(block, mode, summaries, maxSummaryChars) {
...
function projectClaudeBashLines(rawLines, options) {
  ...
  for (const block of candidates) {
    ...
    const placeholder = placeholderLine(block, options.mode, options.summaries, maxSummaryChars);
    const visualRow = rows.length;
    rows.push(Object.freeze({
      visualRow,
